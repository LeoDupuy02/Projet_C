...
int generate_dense_layer(matrix **W, matrix **B, matrix **X, matrix **A ,int nb_layers ,int *size_neurons){
    ...
    for(int i = 0; i < nb_layers; i++){
        ...
        if(i != nb_layers-1){
            ...
            W[i] = init_unit_matrix(size_neurons[i], size_neurons[i]);
        }

        /*B*/
        if(i==0){
            B[i] = init_zero_matrix(size_neurons[i], 1);
        }
        else{
